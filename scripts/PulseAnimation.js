...
function PulseAnimation(bpm) {
    ...
    var bpms = (1/bpm*60)/60000; // 100 beats per 60000 ms (60s, 1m)
    this.speed = (bpms/fpms) * (this.maxRadius - this.minRadius);
}
PulseAnimation.prototype.update = function () {
    this.radius += this.speed;
    if (this.radius > this.maxRadius)
        this.delete = true;
    this.opacity = 1 - (this.radius) / (this.maxRadius);
};
PulseAnimation.prototype.render = function () {
    ctx.save();
    ctx.beginPath();
